...
function ocmx_add_custom_meta(){
	...
	$layout = array(
		...
		"title-display" => array (
			...
			"desc"		=> "Choose whether to enable the Slider or the standard title bar on your widgetized page.",
			"input_type"	=> "select",
	...
	$order = array(
		"orderby" => array (
			...
			"options" 	=> array(__("Post Date",'ocmx') => "date", __("Post Title",'ocmx') => "title", __("Random",'ocmx') => "rand", __("Manual Order",'ocmx') => "menu_order")
		),
		"order" => array (
			"name" 		=> __("order",'ocmx'),
			"default" 	=> __("",'ocmx'),
...
function create_meta_box_ui() {
	global $post, $layout, $portfolio_excerpt, $maps, $features_category, $order, $obox_meta;
...
function insert_obox_metabox($pID) {
	global $post, $maps, $features_category, $obox_meta, $services_meta, $features_meta, $testimonials_meta, $portfolio_meta, $partners_meta, $team_meta, $slider_meta, $meta_added, $layout, $portfolio_excerpt, $order;
